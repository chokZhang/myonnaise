        ...
        for (String name : intParams) {
            if (sp.contains(name)) {
                ...
                if (null != tmp && !"".equals(tmp)) {
                    ...
                    Log.d(TAG, "parseParamArr: name is " + name + "vale is " + Integer.parseInt(tmp));
                }
            }
            //長語音識別
            map.put("vad.endpoint-timeoutvale", 0);
            Log.d(TAG, "parseParamArr: finish " + map.get("vad.endpoint-timeoutvale"));
        }
